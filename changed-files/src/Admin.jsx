...
import {
  CATS, CAT_LABEL, baseForm, formOf, fmtLong,
  toCSV, eventsToCSV, allRegistrationsToCSV, downloadFile,
} from "./lib/helpers.js";
...
function Dashboard({ session, profile, notify, onEventsChanged }) {
  ...
  function exportEvents() {
    if (!events.length) {
      notify("No events to export yet");
      return;
    }
    downloadFile(`eventapp-events-${today()}.csv`, eventsToCSV(events, counts));
    notify(`${events.length} events downloaded`);
  }

  async function exportAllRegistrations() {
    setExporting(true);
    try {
      const rows = await api.listAllRegistrations();
      if (!rows.length) {
        notify("No registrations to export yet");
      } else {
        downloadFile(
          `eventapp-all-registrations-${today()}.csv`,
          allRegistrationsToCSV(events, rows)
        );
        notify(`${rows.length} registrations downloaded`);
      }
    } catch (err) {
      notify(err.message);
    }
    ...
    <>
          ...
          <div className="exportcard">
            <div className="sc-head">
              ...
              <div>
                ...
                <span>Opens in Excel, Google Sheets or Numbers</span>
              </div>
            </div>
            <div className="pp-actions">
              <button className="btn ghost" onClick={exportEvents}>Events CSV</button>
              <button className="btn ghost" onClick={exportAllRegistrations} disabled={exporting}>
                {exporting ? "Preparing…" : "Registrations CSV"}
              </button>
            </div>
            <p className="pp-note">
              Events lists every event with its seats, fee, rules and registration count.
              Registrations puts every person from every event in one sheet.
            </p>
          </div>
      ...
      {view === "regs" && (
        <Registrations events={events} counts={counts} notify={notify} onRefresh={refresh}
          onExportAll={exportAllRegistrations} exporting={exporting} />
      )}
    </>
  );
}
...
function Registrations({ events, counts, notify, onRefresh, onExportAll, exporting }) {
  ...
  return (
    <div className="segbody">
      <div className="sheetcard">
        ...
        <button className="btn ghost wide" onClick={onExportAll} disabled={exporting}>
          <Download size={15} />
          {exporting ? "Preparing…" : "Download every event's registrations"}
        </button>
      </div>
